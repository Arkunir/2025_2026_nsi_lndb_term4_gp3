"""
RL Football Predictor - Version Améliorée (plus précise)
"""
...
class FootballMatchEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Récompense plus stricte
        reward = 2.0 if action == actual else -1.0

        self.current_idx += 1
        done = self.current_idx >= self.end_idx
        terminated = done
        truncated = False
        obs = np.zeros(self.observation_space.shape, dtype=np.float32) if done else self._get_obs(self.current_idx)

        return obs, reward, terminated, truncated, {'idx': self.current_idx - 1, 'actual': actual}
